...
- If \`sap_payments_update_agent\` returns Anchor 3012, \`AccountNotInitialized\`, or \`pricing_menu\`, do not run repair and do not retry the same write. The local bridge worked; this is an SAP registry account lifecycle issue. Call \`sap_agent_next_action\` and \`sap_protocol_invariants\` before deciding the next safe initializer or upgrade path.
