...
- **Prepaid session (recommended):** Call \`sap_payments_start_prepaid\` with \`amountUsd: 1\` to fund a prepaid session. Returns a \`sessionId\`. Pass this as \`prepaidSessionId\` to \`sap_payments_call_paid_tool\` for all subsequent calls — the server grants access via x402 Lifecycle Hooks \`grantAccess\` without per-call 402 challenges. 6x faster, 6x cheaper. Check balance with \`sap_payments_prepaid_balance\` (free).
