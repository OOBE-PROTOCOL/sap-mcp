...
- **Dry-run (free):** Call \`sap_adrena_simulate_position\` to test leverage/collateral combinations without paying. Returns Adrena program logs and would-succeed status.
- **Build (paid):** Call \`sap_adrena_trade_intent\` for the fastest flow — pass \`market\`, \`side\`, \`collateralUsd\`, \`leverage\` (or \`"max"\`), optional \`stopLossPct\`/\`takeProfitPct\`. Resolves mint, decimals, max leverage, collateral token automatically. Returns ready-to-sign tx with balanceCheck and poolMetadata. Alternatively use \`sap_adrena_build_open_short\` / \`sap_adrena_build_open_long\` for explicit control (both accept optional \`stopLossPriceUsd\` for policy compliance). **Do not submit if \`balanceCheck.sufficient\` is false.**
