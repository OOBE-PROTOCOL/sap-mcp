...
| `feature/payments-x402/*` | x402/pay.sh monetization and paid tool bridge | `src/payments/`, `src/tools/x402-paid-call-tool.ts`, `docs/06_X402_PAYSH_MONETIZATION_SETTLEMENT.md` | Pricing source of truth, replay flow, receipt validation, spending caps, recovery text. |
| `feature/protocol-tools/*` | Solana/SAP/DeFi tool families | `src/tools/`, `src/sap/`, `src/perps/`, `src/resources/`, `src/schemas/` | Transaction preview, unsigned builder boundaries, protocol invariants, test fixtures. |
