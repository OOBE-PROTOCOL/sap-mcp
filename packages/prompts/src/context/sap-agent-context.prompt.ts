...
| Adrena perps trading | \`sap_adrena_build_open_long\`, \`sap_adrena_build_open_short\`, \`sap_adrena_build_position_package\`, \`sap_adrena_build_close_long\`, \`sap_adrena_build_close_short\`, \`sap_adrena_build_set_stop_loss\`, \`sap_adrena_build_set_take_profit\`, \`sap_adrena_build_add_liquidity\`, \`sap_adrena_build_swap\`, \`sap_adrena_build_add_limit_order\`, \`sap_adrena_build_trailing_stop\`, \`sap_adrena_build_modify_position\` |
| Risk and signal | \`sap_perp_risk_check\`, \`sap_perp_signal_score\`, \`sap_perp_portfolio_risk\`, \`sap_perp_fear_greed\` |
...
- **Prepaid session (recommended):** Call \`sap_payments_start_prepaid\` with \`amountUsd: 1\` to fund a prepaid session. Returns a \`sessionId\`. Pass this as \`prepaidSessionId\` to \`sap_payments_call_paid_tool\` for subsequent hosted SAP MCP paid calls in the same workflow. The server grants access via x402 Lifecycle Hooks \`grantAccess\`, reducing repeated 402 challenge round-trips while preserving hosted ledger accounting. Check balance with \`sap_payments_prepaid_balance\` (free).
...
- **Risk gate (free):** Call \`sap_perp_risk_check\` with market, side, collateral, leverage. Returns PROCEED/WAIT/BLOCK with risk score 0-1, daily loss, drawdown, cooldown. Do not proceed if recommendation is BLOCK.
- **Signal score (free):** Call \`sap_perp_signal_score\` with mint and market. Returns aggregate score 0-1 with LONG/SHORT/WAIT and confidence. Replaces 5-7 individual indicator calls.
- **Dry-run (free, mandatory before paid builds):** Call \`sap_adrena_oracle_readiness\`, then \`sap_adrena_simulate_position\` for the exact market, side, collateral token, collateral amount, leverage, and pool before any \`sap_adrena_build_*\` or \`sap_adrena_trade_intent\` call. If \`oracleReadiness.ready=false\`, \`wouldSucceed=false\`, or \`simulationError\` is present, stop, explain the specific reason, and do not request approval.
- **Build (paid only after dry-run succeeds):** Call \`sap_adrena_trade_intent\` for the fastest flow — pass \`market\`, \`side\`, \`collateralUsd\`, \`leverage\` (or \`"max"\`), optional \`stopLossPct\`/\`takeProfitPct\`. Resolves mint, decimals, max leverage, collateral token automatically. Returns a signable tx only when \`transactionBase64\` exists, \`safeToApprove=true\`, \`approvalBlocked=false\`, \`simulationStatus="passed"\`, and builder simulation has not returned an on-chain error. Alternatively use \`sap_adrena_build_open_short\` / \`sap_adrena_build_open_long\` for explicit control (both accept optional \`stopLossPriceUsd\` for policy compliance). **Do not submit if \`balanceCheck.sufficient\` is false, \`simulationError\` is present, or \`safeToApprove\` is not true.**
- **Batch (1 payment):** Call \`sap_adrena_build_position_package\` to atomically open + set SL + set TP in 1 transaction. 1 payment, 1 signing, 1 submit instead of 3.
...
- **Leverage:** Pass human-readable leverage (e.g. \`3\` for 3x). The builder encodes it as BPS (30000) internally. Max leverage per market is in \`poolMetadata.maxInitialLeverage\`.
...
- **Adrena error codes in simulation logs:** 6029=MinLeverage, 6071=InsufficientCollateral, 6088=MissingOraclePrice, 6090=CustodyBelowMinimum. Never bypass explicit Anchor/custom simulation errors with \`skipPreflight\`; retry only for expiry/blockhash issues after rebuilding the quote or transaction.
