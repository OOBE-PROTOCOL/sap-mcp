...
- **Dry-run (free, mandatory before paid builds):** Call \`sap_adrena_simulate_position\` for the exact market, side, collateral token, collateral amount, leverage, and pool before any \`sap_adrena_build_*\` or \`sap_adrena_trade_intent\` call. If \`wouldSucceed=false\` or \`simulationError\` is present, stop, explain the specific reason, and do not request approval.
- **Build (paid only after dry-run succeeds):** Call \`sap_adrena_trade_intent\` for the fastest flow — pass \`market\`, \`side\`, \`collateralUsd\`, \`leverage\` (or \`"max"\`), optional \`stopLossPct\`/\`takeProfitPct\`. Resolves mint, decimals, max leverage, collateral token automatically. Returns ready-to-sign tx with balanceCheck and poolMetadata only when builder simulation has not returned an on-chain error. Alternatively use \`sap_adrena_build_open_short\` / \`sap_adrena_build_open_long\` for explicit control (both accept optional \`stopLossPriceUsd\` for policy compliance). **Do not submit if \`balanceCheck.sufficient\` is false or \`simulationError\` is present.**
