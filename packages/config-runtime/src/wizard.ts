 ...
 */
/**
 * @name ensureGlobalBinaryAvailable
 * @description Verifies that the `sap-mcp-server` binary is resolvable from the
 * system PATH. When `npx --package` fails to expose the binary (known npm 10/11
 * bug on macOS), installs the package globally as a fallback so that MCP client
 * configs generated by the repair can use the global binary directly.
 */
function ensureGlobalBinaryAvailable(): void {
  const npmPackage = `@oobe-protocol-labs/sap-mcp-server@${MCP_SERVER_VERSION}`;
  try {
    execFileSync('which', ['sap-mcp-server'], { stdio: 'ignore' });
    return;
  } catch {
    // Binary not in PATH — install globally.
  }
  ...
  try {
    ...
    console.log(`  Installed ${npmPackage} globally for reliable MCP stdio startup.`);
  } catch {
    // Non-fatal: the repair still writes npx-based configs as fallback.
    console.log(`  Warning: could not install ${npmPackage} globally. MCP clients will use npx --package, which may fail on some npm versions.`);
  }
}
...
async function wizardX402PaidCallAddon(): Promise<void> {
  ...
  printInfo('Preferred flow: sap_payments_readiness first, sap_payments_call_paid_tool for paid hosted calls, then sap_payments_finalize_transaction when a hosted builder returns an unsigned transaction. The sap_x402_paid_call name is only a backward-compatible alias.');
  ...
  if (choice === 1) {
    ...
    printInfo('Restart Codex completely, then ask it to use sap_payments.sap_payments_readiness. Use sap_payments.sap_payments_call_paid_tool when a hosted tool requires x402 payment and sap_payments.sap_payments_finalize_transaction for unsigned hosted transactions.');
  }
